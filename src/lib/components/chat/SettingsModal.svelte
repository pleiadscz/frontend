<script lang="ts">
        ...
        import MobileSettingsDrawer from './MobileSettingsDrawer.svelte';
	...
	$: activeLabel = $i18n.t(tabLabels[selectedTab as string] ?? 'Settings');

	const tabClass = (id: string) =>
		`w-full outline-none flex items-center gap-3 px-3 py-2.5 rounded-full text-[14px] text-left transition-colors ${
			selectedTab === id
...
{#if $mobile}
<MobileSettingsDrawer bind:show {getModels} {saveSettings} />
{:else}
<Modal
	size="lg"
	bind:show
	className="bg-[#f9f9f9] dark:bg-[#141414] rounded-[1.5rem] overflow-hidden"
>
	<div
		class="flex text-gray-700 dark:text-gray-100 h-[560px] max-h-[calc(100dvh-4rem)] overflow-hidden"
	>
		<!-- sidebar -->
		<div
			role="tablist"
			id="settings-tabs-container"
			class="tabs w-[200px] flex-shrink-0 flex flex-col py-4 px-2 border-r border-gray-100 dark:border-zinc-800"
		>
			<div class="px-3 mb-4">
				<span class="text-[16px] font-normal text-gray-900 dark:text-white"
			...
			<nav class="flex flex-col gap-0.5 flex-1 overflow-y-auto">
				{#if filteredSettings.length > 0}
					{#each filteredSettings as tabId (tabId)}
						{#if tabId === 'general'}
							...
							>
								<div class="opacity-70"><SettingsAlt strokeWidth="2" /></div>
						...
						{:else if tabId === 'interface'}
							...
							>
								<div class="opacity-70"><AppNotification strokeWidth="2" /></div>
						...
						{:else if tabId === 'connections'}
							{#if $user?.role === 'admin' || ($user?.role === 'user' && $config?.features?.enable_direct_connections)}
								...
								>
									<div class="opacity-70"><Link strokeWidth="2" /></div>
						...
						{:else if tabId === 'tools'}
							{#if $user?.role === 'admin' || ($user?.role === 'user' && $user?.permissions?.features?.direct_tool_servers)}
								...
								>
									<div class="opacity-70"><WrenchAlt strokeWidth="2" /></div>
						...
						{:else if tabId === 'personalization'}
							...
							>
								<div class="opacity-70"><Face strokeWidth="2" /></div>
						...
						{:else if tabId === 'data_controls'}
							...
							>
								<div class="opacity-70"><DatabaseSettings strokeWidth="2" /></div>
						...
						{:else if tabId === 'account'}
							...
							>
								<div class="opacity-70"><UserCircle strokeWidth="2" /></div>
				...
				{#if $user?.role === 'admin'}
					<a
						href="/admin/settings"
						draggable="false"
						class="mt-auto w-full outline-none flex items-center gap-3 px-3 py-2.5 rounded-full text-[14px] text-left transition-colors text-gray-600 dark:text-[#8A8A8D] hover:bg-[#f2f2f2] dark:hover:bg-[#272727] hover:text-gray-900 dark:hover:text-white"
						on:click={async (e) => {
							e.preventDefault();
					...
					>
						<div class="opacity-70"><UserBadgeCheck strokeWidth="2" /></div>
		...
		<!-- content -->
		<div class="flex-1 flex flex-col bg-white dark:bg-[#141414] overflow-hidden">
			<div class="px-7 pt-6 pb-3 flex items-center justify-between">
				<div class="text-[17px] font-normal">{activeLabel}</div>
				<button
					aria-label={$i18n.t('Close settings modal')}
					class="outline-none p-1 rounded-lg hover:bg-gray-100 dark:hover:bg-zinc-800 transition-colors"
					on:click={() => {
						show = false;
					}}
				>
			...
			<div class="h-px bg-gray-100 dark:bg-zinc-800 mx-4"></div>
